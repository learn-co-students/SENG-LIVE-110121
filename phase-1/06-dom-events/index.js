...
// ✅ Handling Click and Submit Events
  ...
  // addEventListener(event, callbackFunction);
    ...
    // of as a function reference?
      ...
      // let targetElement = document.querySelector('#click-me');

      // targetElement.addEventListener("click", saySomething);

      // targetElement.addEventListener("click", () => saySomething("Louis"));
...
// 🚧 Break Out Activity 1: Handling Click Events
  ...
  // 1️⃣ Create a function (deletePokemon) that:
    ...
    function removeLike(pokemon, likesNum){
      likesNum.textContent = --pokemon.likes;
    }
...
// 🚧 Break Out Activity 2: Handling Submit Events
  ...
  // 1️⃣ Create a function (createPokemon) that:
    ...
    //  ✔️ Creates a new Pokemon object and adds it to the "pokemons" Array
      ...
      //  Use .length() to "compute" the final Pokemon's id from "pokemons," incrementing
      //  by one and setting the new value as the "id" for the next new Pokemon object
  ...
  function createPokemon(e){
    ...
    // access input value via click event
    // let pokeName = e.target.children[1].value;
  
    // create new pokemon object
    let newPokemon = {

      // compute length of pokemons array and add 1
